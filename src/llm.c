...
int llm_load(llm_read_fn rd, void *ctx, uint64_t file_size, const char **err) {
    ...
    for (uint64_t i = 0; i < n_kv && !g.failed; i++) {
        ...
        if (str_same(key, "general.architecture") && type == GT_STRING) {
             ...
             * and the suffixes are distinctive enough not to collide
    ...
    /*
     * Two architectures, and they differ in exactly one thing that
     * matters here: qwen2 puts a bias on the q, k and v projections and
     * llama does not. Everything else -- RMSNorm, SiLU, grouped-query
     * attention, rotary embeddings, tied output weights -- is shared,
     * so supporting llama is a matter of letting those biases be
     * absent rather than a second implementation.
     */
...
/*
 * The kernel target is x86_64 with SSE2 on for this file alone, so the
 * square root is one instruction. The guard is for the host: the
 * dequantisation test compiles this same file natively, and an Apple
 * Silicon host cannot assemble sqrtss. Newton-Raphson from a decent
 * seed converges to float32 in four steps and is only ever used there.
 */
