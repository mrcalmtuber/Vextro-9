...
| **Enterprise networking** | LDAP, Kerberos and SMB2 are all here, each proved against a server written independently of the client. What is not: Group Policy and remote desktop. No VPN and no branch caching either. Two limits worth stating: Kerberos holds tickets in memory only, so nothing survives a reboot; and SMB2 signs but does not encrypt — SMB 3.0 encryption is not implemented. |
...
**64,989 lines of C written here**, across 124 files, compiled as a
single translation unit plus one for inference, over a user-space C
library of its own. (68,604 counting the embedded typeface, the integer
